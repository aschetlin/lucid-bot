...
class Announce(commands.Cog):
    ...
    async def _announce(self, ctx: commands.Context, *args) -> None:
        if not args:
            ...
            await message.edit(embed=embed)
            embedDescription = await self.nbf.yes_no_dialogue(message, ctx, 20, False)
            ...
            while True:
                ...
                try:
                    image_message: discord.Message = await self.bot.wait_for(
                        "message", timeout=40
                    )
                ...
                if image_message.author.id == ctx.author.id:
                    break
            ...
            announceEmbed.set_image(url=image_message.content)
